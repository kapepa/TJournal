...
const LayoutDefault: NextPage<ILayoutDefault> = ({title, query, children }) => {
  ...
  const { detailed, short } = useSelector(( store: any ) => store.article);
  ...
  console.log(useSelector(( store: any ) => store.article))
  return (
    <DataContext.Provider value={{short, article: detailed , user: profile, query, win, wrong: wrongActive}}>
      <Head>
        <title>{title}</title>
